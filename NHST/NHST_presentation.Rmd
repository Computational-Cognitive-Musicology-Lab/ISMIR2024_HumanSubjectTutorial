...
So far, we've happily made the "normal assumption" that the residual our dependent variable is normally distributed, with a fixed variance.

+ Both *ANOVA* and *Multiple Regression* assume this.
+ Luckily, these tests are known to be "robust" to minor violations of these assumptions.
+ However, we need to be vigilant!

---

We might hypothesize that the emotion *fear* is caused by tension. 

+ This could be modeled as another (multiple) linear regression.
+ However...


----

The fear ratings are severely clustered at 1 (`r paste0(round(mean(emote$fear == 1)*100), '%')` of ratings).

```{r}

with(emote, draw(tension, fear, lm = T))

```

+ As skeptics, we should question whether it makes sense to model these variables as a linear relationship.
+ We might consider alternative ways of characterizing this relationship.

---

A simple approach would be the recharacterize our fear variable as a binary variable: 1, or not 1.

```{r}

with(emote, draw(tension, fear, lm = T, col = ifelse(fear >1, 'red', 'black'), alpha=.6))
...
emote$AnyFear <- emote$fear > 1

logFit <- glm(AnyFear ~ energy + tension + valence + anger +  happy + sad + tender, 
              data = emote, family = binomial())
...
anova(logFit, test = 'Chisq') |> knitr::kable()
```
...
```{r}
par(bg = '#ffffff00')
with(emote, plot(tension, as.integer(AnyFear), pch = 16, axes = FALSE, xlim=c(1,9),
                 col = ifelse(AnyFear, 'red', 'black'), xlab = 'Tension rating', ylab = "Fear or Not"))
axis(1, seq(1, 9), tick = FALSE)
axis(2, c(0, 1), c('Not fear', 'Fear'), tick = FALSE, las = 1)
