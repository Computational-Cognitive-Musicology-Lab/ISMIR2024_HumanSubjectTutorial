...
+ What kind of variable? What does error look like?
  + Continuous, real numbers?
    + Linear regression, $t$-test, ANOVA (with normal error)
  + Categories
...
+ Start with basic ("baby step") examples, before finishing with a review of real *best practices*.
+ To follow along:
  + https://posit.cloud/content/8798770 (Need to make a free account.)


## Quality Evaluation Data

```{r, echo = 1}
SiSEC <- read.csv('../../Data/SiSEC08_source_separation/SiSEC08_parsed.csv') |> subset(grepl('Algo', Algorithm))
...
```{r, echo = 1}
emote <- fread('../../Data/Eerola_emotion/mean_ratings_set2.csv')
```
...
#     mtext(bquote(t[102] == .(round(Ts, 2))), 1, cex = 1.32, line = 6, xpd =T)
#   }
# 
# })
...
#     Ts <- coef(summary(fit))['Tension', 't value']
#     par(bg = '#ffffff00', mar = c(2,4,0,4))
#     curve(dt(x, 102), -5, 5, n = 1000, col = 'blue', xlim = c(-5, 5),
#           xlab = quote(t[102]), ylab = quote(paste(Density, '|', H[0])),
#           axes = FALSE)
#     points(Ts, dt(Ts, 102), col = 'red', pch = 4)
#     if (i > 1) points(allts, dt(allts, 102), col = 'grey50', cex=.5, pch = 4)
#     allts <- c(allts, Ts)
...
par(bg = '#ffffff00')
curve(dt(x, 102), -5, Ts, n = 1000, col = 'blue', xlim = c(-5, 15),
      xlab = quote(t[102]), ylab = quote(paste(Density, '|', H[0])),
      axes = FALSE)
curve(dt(x, 102), Ts, 100, n = 1000, col = 'red', xlim = c(0, 100), add =TRUE)
axis(1, seq(-4, 14, 2), tick = FALSE)
axis(2, seq(0, 1, .2), tick = FALSE, las = 1)

points(Ts, dt(Ts, 102), col = 'red')
```
...
emote_valence$POSITIVE <- emote_valence$TARGET %in% c("HAPPY_HIGH", "TENDER_HIGH", 
                                                      "VALENCE POS HIGH", "ENERGY POS HIGH", 
                                                      "TENSION POS HIGH" )
...
+ $H_0$ is that the there is no relationship between the emotion ratings and target valence.
...
```{r}
par(bg = '#ffffff00')
with(emote_valence, plot(Valence, as.integer(POSITIVE), pch = 16, axes = FALSE, xlim=c(1,9),
                 col = ifelse(POSITIVE, 'blue', 'black'), xlab = 'Valence rating', ylab = "Valence of Target"))
axis(1, seq(1, 9), tick = FALSE)
axis(2, c(0, 1), c('Negative', 'Positive'), tick = FALSE, las = 1)

logFit <- glm(POSITIVE ~ Anger  + Valence + Tension + Fear + Happy + Sad + Tender + Energy,
    family = binomial(),
    data = emote_valence)

xs <- seq(1,9,.01)
newdata <- with(emote_valence, data.frame(Tension = mean(Tension),
                                  Fear = mean(Fear),
                                  Energy = mean(Energy),
                                  Valence = xs,
                                  Anger = mean(Anger),
...
```{r}
emote_ours <- fread('../../Data/ISMIR2024_data/emotion_data')
