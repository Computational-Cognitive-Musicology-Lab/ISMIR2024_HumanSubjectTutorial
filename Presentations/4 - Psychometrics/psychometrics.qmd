...
Like the 1PL binary model, the partial credit model also has specific objectivity. When the distance between scale points is restricted to be the same for every item, we call this the **rating scale model**.
...
- There are two major approaches to factor analysis.
  ...
  - **Confirmatory factor aanlysis** (CFA)
...
gold_cfa <- 
  cfa(
    model = "f1 =~ Gold_1_1 + Gold_1_2 + Gold_1_3
